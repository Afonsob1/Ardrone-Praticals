...
bool getCameraParameters(ros::NodeHandle& nh, double& fu, double&fv, double& cu, double& cv, double& k1, double& k2, double& p1, double&p2, double& mapFocalLength, int& maxPixelDistance, int& numPosesToMatch){
  ...
  ROS_INFO("maxPixelDistance: %d", maxPixelDistance);
  ROS_INFO("numPosesToMatch: %d", numPosesToMatch);
  return success;
}
  ...
  double& LAND_HEIGHT)
{
      ...
      waypoints.back().yaw = 0;
      waypoints.back().z = FLYING_HEIGHT;
      
      waypoints.push_back(arp::Autopilot::Waypoint{waypoints.back().x, waypoints.back().y, LAND_HEIGHT, 0, THRESHOLD_LANDING, true});
...
int main(int argc, char **argv)
{
  ...
  arp::Frontend frontend(640, 360, fu, fv, cu, cv, k1, k2, p1, p2,
                          ...
                          maxPixelDistance, numPosesToMatch);
  ...
  // tell estimator to call autopilot's controller
  visualInertialTracker.setCheckLandedCallback(
        std::bind(&arp::Autopilot::checkLandedCallback, &autopilot,
        std::placeholders::_1));
